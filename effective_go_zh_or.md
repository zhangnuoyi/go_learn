...
Go 是一门全新的编程语言。虽然它借鉴了现有语言（如 C、Pascal 和其他语言）的思想，但它具有独特的特性，使得高效的 Go 程序在本质上不同于其他语言编写的程序。简单地将 C++ 或 Java 程序翻译成 Go 代码不太可能产生令人满意的结果——Java 程序是用 Java 编写的，而不是 Go。另一方面，从 Go 的角度思考问题可能会产生成功但完全不同的程序。

换句话说，要写好 Go 代码，了解它的特性和惯用写法非常重要。同时，了解 Go 编程的既定约定也很重要，如命名、格式化、程序构造等，这样你编写的程序会更容易被其他 Go 程序员理解。

本文档提供了编写清晰、简洁、有效率的 Go 代码的指南。
...
我们使用制表符进行缩进，gofmt 默认会输出制表符。除非必须，否则不要使用空格。

### 行长度

Go 没有行长度限制。不用担心溢出穿孔卡片。如果一行感觉太长，就换行并用额外的制表符缩进。

### 括号

Go 比 C 和 Java 需要更少的括号：控制结构（if、for、switch）的语法中没有括号。此外，操作符优先级层次更短更清晰，所以
```go
x<<8 + y<<16
```
...
Go 的注释是为了帮助读者理解代码，而不仅仅是描述代码的作用。它们应该解释为什么代码会这样工作，或者它解决了什么问题，而不是简单地重述代码本身。
...
Effective Go 是学习 Go 语言高效编程的重要资源。本文档提供了一份中文翻译，帮助开发者快速掌握 Go 语言的核心概念和惯用写法。
